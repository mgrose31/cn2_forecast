...
rmse_train_MLP_std = np.std(rmse_train_MLP, axis=1, ddof=1)
rmse_train_RNN_std = np.std(rmse_train_RNN, axis=1, ddof=1)
rmse_train_GRU_std = np.std(rmse_train_GRU, axis=1, ddof=1)
rmse_train_LSTM_std = np.std(rmse_train_LSTM, axis=1, ddof=1)

rmse_valid_MLP_std = np.std(rmse_valid_MLP, axis=1, ddof=1)
rmse_valid_RNN_std = np.std(rmse_valid_RNN, axis=1, ddof=1)
rmse_valid_GRU_std = np.std(rmse_valid_GRU, axis=1, ddof=1)
rmse_valid_LSTM_std = np.std(rmse_valid_LSTM, axis=1, ddof=1)
...
#%%
plt.figure()
plt.plot(rmse_valid_mean_MLP_sorted, '-', label='MLP')
plt.plot(rmse_valid_mean_RNN_sorted, '-', label='RNN')
plt.plot(rmse_valid_mean_GRU_sorted, '-', label='GRU')
plt.plot(rmse_valid_mean_LSTM_sorted, '-', label='LSTM')
plt.title('Average Model Performance')
plt.xlabel('sorted index')
plt.ylabel('$log_{10}(C_{n}^{2})$ validation loss')
plt.grid(True)
plt.grid(True, which='minor')
plt.legend(loc='best')
plt.tight_layout()
...
plt.figure()
plt.errorbar(x=range(10), y=rmse_valid_mean_MLP_sorted[:10],
             yerr=rmse_valid_sdom_MLP_sorted[:10], fmt='-o', label='MLP')
plt.errorbar(x=range(10), y=rmse_valid_mean_RNN_sorted[:10],
             yerr=rmse_valid_sdom_RNN_sorted[:10], fmt='-o', label='RNN')
plt.errorbar(x=range(10), y=rmse_valid_mean_GRU_sorted[:10],
             yerr=rmse_valid_sdom_GRU_sorted[:10], fmt='-o', label='GRU')
plt.errorbar(x=range(10), y=rmse_valid_mean_LSTM_sorted[:10],
             yerr=rmse_valid_sdom_LSTM_sorted[:10], fmt='-o', label='LSTM')
plt.ylim(0.18, 0.195)
plt.title('Average Model Performance')
plt.xlabel('sorted index')
plt.ylabel('$log_{10}(C_{n}^{2})$ validation loss')
...
# data_B = np.transpose(rmse_valid_GRU_sorted[0:,:])
# data_B = np.transpose(rmse_valid_RNN_sorted[0:,:])
# data_B = np.transpose(rmse_valid_LSTM_sorted[0:,:])
# t, p = stats.ttest_ind(data_A, data_B, equal_var=False)

# data_A = rmse_valid_GRU_sorted[0,:]
# data_B = np.transpose(rmse_valid_MLP_sorted[0:,:])
...
data_A = rmse_valid_GRU_sorted[0,:]
data_B_MLP = np.transpose(rmse_valid_MLP_sorted[0:,:])
data_B_RNN = np.transpose(rmse_valid_RNN_sorted[0:,:])
data_B_GRU = np.transpose(rmse_valid_GRU_sorted[1:,:])
data_B_LSTM = np.transpose(rmse_valid_LSTM_sorted[0:,:])
_, p_MLP = stats.ttest_ind(data_A, data_B_MLP, equal_var=False)
_, p_RNN = stats.ttest_ind(data_A, data_B_RNN, equal_var=False)
_, p_GRU = stats.ttest_ind(data_A, data_B_GRU, equal_var=False)
_, p_LSTM = stats.ttest_ind(data_A, data_B_LSTM, equal_var=False)

plt.figure()
plt.plot(range(10), p_MLP[:10], '-o', label='MLP')
plt.plot(range(10), p_RNN[:10], '-o', label='RNN')
plt.plot(range(1, 10), p_GRU[:9], '-o', label='GRU')
plt.plot(range(10), p_LSTM[:10], '-o', label='LSTM')
plt.plot(range(10), np.ones(10)*alpha, 'k--', label=r'$\alpha$ = 0.05')
plt.title("Sudent's t-test")
plt.xlabel('sorted index')
plt.ylabel('two-tailed p-value')
plt.grid(True)
plt.grid(True, which='minor')
plt.legend(loc='best')
plt.tight_layout()
